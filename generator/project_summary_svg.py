...
EV_ICON = ('if(contains(?typeSFX, "combinatorial"), "\U0001F9E9",'
...
KNO_ICON = 'if(?typeSFX = "first evidence", "\U0001F331", "\U0001F4C8")'
...
{valid('fg' + s, pi=False, member=member)}
          graph ?afg{s} {{
            ?fnd{s} a hycl:AIDA-Sentence ; cito:obtainsSupportFrom ?studyg{s} .
          }}
{valid('rg' + s, pi=False, member=member)}
          graph ?arg{s} {{
            {item} hycl:hasMoreGeneralMeaningThan ?fnd{s} .
          }}"""
        meta = f"""          optional {{
...
{valid('pg' + s, pi=False, member=False)}
          ...
          bind(if(bound(?oproj{s}), "increased evidence", "first evidence") as ?type{s})
          bind(0 as ?rr{s})
          bind({ic} as ?icon{s})"""
...
{valid(s, member=member)}
          graph ?a{s} {{
            {item} a obo:ECO_0000000 ; prov:wasGeneratedBy ?study{s} .
          }}"""
        meta = f"""          optional {{ graph ?a{s} {{ {item} rdfs:label ?la{s} }} }}
          ...
          bind(coalesce(?subl{s}, "evidence") as ?type{s})
          bind(0 as ?rr{s})
          bind({ic} as ?icon{s})"""
    elif s == "act":
